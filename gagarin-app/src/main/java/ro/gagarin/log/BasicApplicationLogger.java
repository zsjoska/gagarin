...
public class BasicApplicationLogger implements AppLog {

    /**
     * The fully qualified name of the Category class.
     */
    private static final String FQCN = BasicApplicationLogger.class.getName();
    ...
    public BasicApplicationLogger(Session session, Class<?> aClass) {
	...
	addLogEntryIfPermitted(Level.DEBUG, message + " Exception:" + t.getMessage());
    }
	...
	addLogEntryIfPermitted(Level.INFO, message + " Exception:" + t.getMessage());
    }
	...
	addLogEntryIfPermitted(Level.ERROR, message + " Exception:" + t.getMessage());
    }
	...
	System.err.println(session + "@ " + action.name() + "# " + id + ": " + classInAction.getSimpleName() + ": "
		+ detail);
    }
