...
public interface AuthorizationManager extends BaseManager {
    ...
    /**
     * Verifies if the current logged in user satisfies one of the permission
     * test.<br>
     * The given list is verified against the current effective permission set
     * of the current session<br>
     * The permission test list provided is considered with OR operation. If
     * more permissions are required (AND would be required) an additional
     * method call should be performed.
     * 
     * @param session
     *            current session
     * @param ce
     ...
     * @throws OperationException
     */
    void requiresPermission(Session session, PermissionTest... tests) throws PermissionDeniedException,
	    OperationException;
