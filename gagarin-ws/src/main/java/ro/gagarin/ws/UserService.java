...
public class UserService {
	...
	private static final transient ManagerFactory FACTORY = BasicManagerFactory
			.getInstance();

	@WebMethod
	public Long createUser(String sessionId, WSUser user)
		...
		AuthorizationManager permissionManager = FACTORY
				.getAuthorizationManager(session);

		try {

			// the session user must have CREATE_USER permission
			permissionManager.requiresPermission(session,
			...
			if (role.getId() == null && role.getRoleName() != null) {
				...
				if (role == null) {
					userManager.markRollback();
					throw new ItemNotFoundException(UserRole.class, user
							.getRole().getRoleName());
				}
				user.setRole(role);
			}
	...
	@WebMethod
	public List<WSUserRole> getRoleList(String sessionId)
			throws SessionNotFoundException, PermissionDeniedException,
			OperationException {
	...
	@WebMethod
	public WSUserRole createRoleWithPermissions(String sessionId,
			String roleName, WSUserPermission[] permissions)
			throws SessionNotFoundException, PermissionDeniedException,
		...
		try {
			...
			LOG.debug("Create role:" + roleName + " with permissions "
					+ Arrays.toString(permissions));

			// the session user must have LIST_PERMISSIONS permission
			permissionManager.requiresPermission(session,
					PermissionEnum.LIST_PERMISSIONS);
			List<UserPermission> allPermissions = roleManager
					.getAllPermissions();
			List<UserPermission> matched;
			matched = ConversionUtils.matchPermissions(allPermissions,
					permissions);
	...
	public List<WSUserPermission> getAllPermissionList(String sessionId)
			throws SessionNotFoundException, OperationException,
	...
	@WebMethod
	public List<WSUserPermission> getRolePermissions(String sessionId,
			WSUserRole wsUserRole) throws SessionNotFoundException,
		...
		try {
			...
			Set<UserPermission> permissions = roleManager
					.getRolePermissions(wsUserRole);
	...
	@WebMethod
	public void deleteRole(String sessionId, WSUserRole role)
	...
	@WebMethod
	public List<WSConfig> getConfigEntries(String sessionId)
		...
		try {
			...
			FACTORY.getAuthorizationManager(session).requiresPermission(
					session, PermissionEnum.LIST_CONFIG);

			ConfigurationManager cfgMgr = FACTORY.getConfigurationManager();
			List<ConfigEntry> configValues = cfgMgr.getConfigValues();
			List<WSConfig> wsConfigList = WSConversionUtils
					.toWSConfigList(configValues);
			return wsConfigList;
		} finally {
			FACTORY.releaseSession(session);
		}
	...
	@WebMethod
	public void setConfigEntry(String sessionId, WSConfig wsConfig)
			throws SessionNotFoundException, OperationException, PermissionDeniedException {
		SessionManager sessionManager = FACTORY.getSessionManager();
		Session session = sessionManager.acquireSession(sessionId);
		...
		try {
			...
			FACTORY.getAuthorizationManager(session).requiresPermission(
					session, PermissionEnum.UPDATE_CONFIG);
	...
	@WebMethod
	public List<WSLogEntry> getLogEntries(String sessionId, String user)
			throws SessionNotFoundException, OperationException, PermissionDeniedException {
		SessionManager sessionManager = FACTORY.getSessionManager();
		Session session = sessionManager.acquireSession(sessionId);
